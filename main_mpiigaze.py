...
# Custom
import models.lossconvnet as lossnet
...
run = neptune.init(
    project="wonderit/mpiigaze-ll4al",
...
unlabeled_dataset, _ = create_dataset(config, True)
test_dataset = create_dataset(config, False)
dataset_size = {'train': len(train_dataset), 'test': len(test_dataset)}



run['config/hyperparameters'] = PARAMS
run['config/configs'] = config.dump()
run["config/dataset/path"] = output_dir
...
def train_epoch(models, criterion, optimizers, dataloaders, epoch, epoch_loss, cycle, trial):
    ...
    if PARAMS['is_kd'] and cycle > 0:
        prev_cycle = cycle - 1
        teacher_model_path =f'{output_dir}/teacher_model_trial{trial}_cycle{prev_cycle}_server{PARAMS["server"]}.pth'
        # models['teacher_backbone'] = lenet.ResNet18(num_classes=24)
        models['teacher_backbone'] = create_model(config)
    ...
    for data in tqdm(dataloaders['train'], leave=False, total=len(dataloaders['train'])):
        images = data[0].to(PARAMS['device'])
        poses = data[1].to(PARAMS['device'])
        gazes = data[2].to(PARAMS['device'])
        iters += 1

        optimizers['backbone'].zero_grad()
        optimizers['module'].zero_grad()

        scores, features = models['backbone'](images.float(), poses.float())
        target_loss = criterion(scores, gazes)
        ...
        if models.get('have_teacher', False):
            teacher_outputs, teacher_feature = models['teacher_backbone'](images.float(), poses.float())
...
def test(models, dataloaders, mode='val'):
    ...
    with torch.no_grad():
        ...
        pred_array = []
        labels_array = []
        for (images, poses, gazes) in dataloaders[mode]:
            images = images.to(PARAMS['device'])
            poses = poses.to(PARAMS['device'])
            gazes = gazes.to(PARAMS['device'])
            scores, _ = models['backbone'](images.float(), poses.float())

            pred_array.extend(scores.cpu().numpy().reshape(-1))
            labels_array.extend(gazes.cpu().numpy().reshape(-1))
            total += gazes.size(0)
...
def get_uncertainty(models, unlabeled_loader):
    ...
    with torch.no_grad():
        for (images, poses, gazes) in unlabeled_loader:
            images = images.to(PARAMS['device'])
            poses = poses.to(PARAMS['device'])
...
if __name__ == '__main__':
    ...
    for trial in range(PARAMS['trials']):
        ...
        # Model
        convnet     = create_model(config)
        # resnet18    = resnet.ResNet18(num_classes=2).to(PARAMS['device'])
        loss_module = lossnet.LossNet().to(PARAMS['device'])
        models      = {'backbone': convnet, 'module': loss_module}
        ...
        for cycle in range(PARAMS['cycles']):
            ...
            if PARAMS['is_random']:
                ...
                unlabeled_loader = DataLoader(unlabeled_dataset, batch_size=PARAMS['batch_size'],
            ...
            dataloaders['train'] = DataLoader(train_dataset, batch_size=PARAMS['batch_size'],
                                              sampler=SubsetRandomSampler(labeled_set),
                                              pin_memory=True)
            # Save a checkpoint
            torch.save({
                    ...
                    f'{output_dir}/teacher_model_trial{trial}_cycle{cycle}_server{PARAMS["server"]}.pth')
